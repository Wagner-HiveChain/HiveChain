...
```
Stay tuned for official release notes and updates on installation details.

### 2. Run a Quick AI Workflow
```python
from hivechain import Agent, Workflow

# Define a simple agent
my_agent = Agent(model="openai:gpt-3.5")

# Create a workflow that queries multiple models
workflow = Workflow(
    agents=[my_agent],
    memory="local",
    alignment_mode="soft_prompts"
)

# Execute the workflow
response = workflow.run("Explain quantum entanglement in simple terms.")
print(response)
```
